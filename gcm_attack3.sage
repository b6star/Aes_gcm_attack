...
# TLS 1.2 AAD 길이
TLS_AAD_LENGTH = 13
aad1_length = aad2_length = TLS_AAD_LENGTH

# Ciphertext 길이
ciphertext1_length = 34

ciphertext2_length = 50


AAD1_bytes = bytes.fromhex("""
    00 00 00 00 00 00 00 03
    17 03 03 00 22
""")

AAD2_bytes = bytes.fromhex("""
...
attack_polynomial = build_attack_polynomial(
    AAD1_bytes,
    C1_bytes,
    block_to_field(T1_bytes),
    AAD2_bytes,
    C2_bytes,
    block_to_field(T2_bytes)
)
...
for index, candidate_H in enumerate(
    ...
    print(
        "패킷 2 태그와 일치하는가?:",
        predicted_T2 == block_to_field(T2_bytes)
    )
